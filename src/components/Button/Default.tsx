...
export const DefaultButton: FunctionComponent<PropsWithChildren<ButtonProps>> = ({ children, className = "", loading, loadingColor = "bg-black", ...props }) => {
	return (
		<button className={cc(className, "rounded font-bold px-6 py-3 transition-all active:scale-95 flex items-center justify-center h-[48px] shadow hover:scale-105 disabled:hover:scale-100 relative overflow-hidden cursor-pointer")} {...props}>
			{loading && (
