...
export const TextArea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(function TextArea({ description, error, label, ...props }, ref) {
	...
	const border = error ? "border-red-500 peer-focus:border-red-500" : "border-gray-200 peer-focus:border-black";
	...
	return (
		<InputWrapper description={description} error={error} displayMessage={true} cursor="text" height="h-full">
			...
			/>
			;
			<label htmlFor={props.id} className={cn(labelColor, labelPosition, "absolute z-20 origin-left text-base duration-300")}>
				{label}
			</label>
			<div className={cn(background, border, borderRadius, "absolute inset-0 z-0 h-full w-full border transition-all peer-focus:rounded-2xl peer-focus:border-2")} />
		</InputWrapper>
	);
});
